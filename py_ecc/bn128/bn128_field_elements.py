...
class FQP():
   ...
   # I am rewriting this code from recursive definition to
    # iterative one because it is causing my program to crash
    # possibly due to stack overflow in long exponentiation
    # particularly in Fq12 where the order is q**12

    # This operation is not timing safe and is not to be used in timing
    ...
    def __pow__(self, other):
        if other == 0:
            ...
            power = self.__class__(self.coeffs) #  copy self as power
            cumul = self.__class__([1] + [0] * (self.degree - 1)) # identity in Fq12
            while other > 0:
                if other % 2:
                    cumul = cumul * power
                    other = other - 1
